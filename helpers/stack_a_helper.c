...
{
	if (ps->chunk_to_sort.size == 1 && ft_is_sorted(&ps->stack_a.head) \
		...
		(&ps->stack_b.head, ps->stack_a.head->data) && is_the_grtst_from_head \
		(&ps->stack_b.head, ps->stack_a.head->next->data))
	{
...
void retrieve_chunk_to_sort_of_a(t_push_swap *ps)
{
	...
	if (ft_is_sorted(&ps->stack_a.head) == 0)
	{
