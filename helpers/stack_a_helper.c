...
{
    if (ps->chunk_to_sort.size == 1 && ft_is_rev_sorted(&ps->stack_a.head) && is_the_greatest_from_head(&ps->stack_b.head, ps->stack_a.head->data))
        ps->stack_a.head->chunk = 0;
    else if (ps->chunk_to_sort.size == 2 && is_the_greatest_from_head(&ps->stack_b.head, ps->stack_a.head->data) && is_the_greatest_from_head(&ps->stack_b.head, ps->stack_a.head->prev->data))
    {
...
void retrieve_chunk_to_sort_of_a(t_push_swap *ps, int *flg_a, int *flg_b)
{
    ...
    if (ft_is_rev_sorted(&ps->stack_a.head) == 0)
    {
        ...
        {
            get_elements_reversed_of_stack_a_to_top(ps, ps->stack_a.tail->chunk);
            get_chunk_to_sort_from_tail(&ps->chunk_to_sort, ps->stack_a.head->chunk, ps->stack_a);
        }
        else
            get_chunk_to_sort_from_tail(&ps->chunk_to_sort, ps->stack_a.head->chunk, ps->stack_a);
    }
