...
void retrieve_chunk_to_sort_of_b(t_push_swap *ps)
{
    ...
    get_chunk_to_sort_from_tail(&ps->chunk_to_sort, ps->stack_b.head->chunk, ps->stack_b);
    if (ps->chunk_to_sort.size > 2 && ps->stack_b.head->data < ps->stack_b.tail->data)
    {
        get_elements_reversed_of_stack_b_to_top(ps, ps->stack_b.tail->chunk);
        get_chunk_to_sort_from_tail(&ps->chunk_to_sort, ps->stack_b.head->chunk, ps->stack_b);
    }
    return;
}
...
{
    if (ft_is_rev_sorted(&ps->stack_a.head) && ps->stack_b.head->data >= ps->stack_b.tail->data)
    {
        if (ps->stack_b.size == 2 || ps->stack_b.size == 1)
            handle_stack_b_case_of_less_nbrs(ps);
        else if (ps->chunk_to_sort.size == 2 && is_the_greatest_from_head(&ps->stack_b.head->prev->prev, ps->stack_b.head->data) && is_the_greatest_from_head(&ps->stack_b.head->prev->prev, ps->stack_b.head->prev->data))
            handle_stack_b_case_of_less_nbrs(ps);
        else if (ps->chunk_to_sort.size == 1 && is_the_greatest_from_head(&ps->stack_b.head->prev, ps->stack_b.head->data))
            handle_stack_b_case_of_less_nbrs(ps);
        else
        {
