...
/*   Updated: 2022/10/10 05:11:31 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_stack_a_sorting_process(t_push_swap *ps, int *flg_a, int *flg_b)
{
    ...
    while (*flg_a && *flg_b == 0 && ft_is_rev_sorted(&ps->stack_a.head) == 0)
    {
        ...
        if (ps->stack_a.tail->chunk != 0)
            get_elements_reversed_of_stack_a_to_top(ps, ps->stack_b.tail->chunk);
        check_flag_status(ps, flg_a, flg_b);
    }
    return;
}
...
{
    while (ps->stack_a.head->data < ps->middle->value && ps->stack_a.head->chunk == ps->chunk_to_sort.tail->chunk && *flg_a)
    {
...
void is_chunk_a_already_sorted(t_push_swap *ps, int *flg_a, int *flg_b)
{
    ...
    {
        while (ft_is_rev_sorted(&ps->chunk_to_sort.head))
        {
